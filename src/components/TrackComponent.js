...
const TrackComponent = ({track}) => {
  ...
  return (
    <article className='max-w-10/12 w-fit m-auto bg-blue-600 border drop-shadow-md border-black'>
        ...
        {tracksContext.selectedItem && !itemsHidden && tracksContext.selectedItem.filePath
        ? <CodeExample category={tracksContext.selectedCategory} item={tracksContext.selectedItem} />
        : ''
        }
    </article>
  )
